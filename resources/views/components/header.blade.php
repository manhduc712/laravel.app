            ...
            <div id="mega-menu-full"
                ...
                <ul
                    ...
                    <li class="md:col-span-2">
                        <a href="{{ route('home') }}"
                            class="cursor-pointer font-medium inline-flex items-center py-5 none-underline hover:underline uppercase">Home</a>
                    </li>
                    <li class="md:col-span-2">
                        <a href="{{ route('product') }}"
                            class="cursor-pointer font-medium inline-flex items-center py-5 none-underline hover:underline uppercase">Product</a>
                    </li>
                    @if (session()->has('user_info'))
                        @if (session()->get('user_info')['department_id'] == '1')
                            <li class="md:col-span-2">
                                <a href="{{ route('account') }}"
                                    class="cursor-pointer font-medium inline-flex items-center py-5 none-underline hover:underline uppercase">Account</a>
                            </li>
                        @endif
                ...
                <div class="place-items-end place-self-center  w-full lg:flex md:w-auto md:order-1 lg:grid-cols-1 ">
                    <ul
                        ...
                        @if (session()->has('user_info'))
                            <li>
                                <span class="block mt-2 uppercase">{{ session()->get('user_info')['username'] }}</span>
                            </li>
                            <li>
                                <a href="{{ route('logout') }}"
                                    class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded">Log out</a>
                            </li>
                        @else
                            <li>
                                <a href="{{ route('login') }}" class="block mt-2 none-underline hover:underline">Log
                                    In</a>
                            </li>
                            <li>
                                <a href="#" class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded">Sign
                                    up</a>
                            </li>
                        @endif
                    </ul>
